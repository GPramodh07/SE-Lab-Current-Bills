...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    if (mysqli_num_rows($check) > 0) {
        echo "<script>alert('Supervisor already exists');</script>";
        exit;
    }

    $hashed_password = password_hash($password, PASSWORD_DEFAULT);

    $q1 = "INSERT INTO supervisors (name,email,phone,address,pincode)
           VALUES ('$name','$email','$phone','$address','$pincode')";

    if (!mysqli_query($conn, $q1)) {
        echo "<script>alert('Failed to create supervisor');</script>";
        exit;
    }
    ...
    if (!mysqli_query($conn, $q2)) {
        echo "<script>alert('Failed to create supervisor login');</script>";
        exit;
    }
